                            ...
                            <form enctype="multipart/form-data" action="{{route('backend.stock.store')}}" method="POST">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-9">
                                             <select id="select_option_value" class="form-control" name="select_option_value" required onchange="findPreviousItemsas()">
                                                    <option value="">--Select Variant Value--</option> 
                                            </select> 
                                            <p class="text-danger" style="display: none;" id="selectedItemError">You are already added this item select other item or Variant</p>
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="text-end mb-3">
                                                <button class="btn btn-primary" id="saveVariant">Save</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
<script> 
    $(document).ready(function(){
        $('#select_variant_type').prop('disabled', true);
        $('#select_option_value').prop('disabled', true);

        $("#select_product, #select_variant_type").on('change', function(){ 
            let select_product_val = $('#select_product').val();
            let select_variant_type_val = $('#select_variant_type').val(); 
            if(select_product_val !== ""){
                $('#select_variant_type').prop('disabled', false);
                
                if(select_variant_type_val !== ""){
                     $('#select_option_value').prop('disabled', false);
                }else{
                   $('#select_option_value').prop('disabled', true);
                } 
            }else{
 ...
 $(document).on("change", "#select_variant_type", selectVariantType); 
  ...
  const findPreviousItemsas = async () =>{

        // check Previous items 

        let select_variant_type = $('#select_variant_type').val(); 
        let select_variant_value = $('#select_option_value').val();
        let select_product = $('#select_product').val();
  
        const url = "{{route('backend.stock.check_product_in_variant')}}";
        const previousItemResponse = await fetch(`${url}?product_id=${select_product}&variant_type=${select_variant_type}&variant_value=${select_variant_value}`);
        try{
            const myresponse = await previousItemResponse.json();
            
            if(myresponse.availablity === false){
                $('#selectedItemError').show();
                $('#saveVariant').prop('disabled', true);
                $('#saveVariant').css('background', "#e9ecef");
            }else{
                $('#selectedItemError').hide();
                $('#saveVariant').prop('disabled', false);
                $('#saveVariant').css('background', "#f5a100")
            }
